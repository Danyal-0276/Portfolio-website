...
export const socialLinks: SocialLink[] = [
  ...
  },
  {
    ...
    handle: "danyal-tanveer-30b887320",
    icon: "linkedin",
  },
];
...
export function SocialLinks({ variant = "default", className }: SocialLinksProps) {
  ...
  if (variant === "hero") {
    return (
      <div className={cn("hero-social flex flex-wrap gap-3", className)}>
        {socialLinks.map((link) => {
          const Icon = icons[link.icon];
          return (
            <a
            ...
            >
              ...
              <span className="hidden text-charcoal/60 sm:inline">{link.handle}</span>
            </a>
          );
        })}
      </div>
      ...
      {socialLinks.map((link) => {
        ...
        return (
          ...
          >
            ...
            <span>
              <span className="block font-medium text-charcoal">{link.name}</span>
